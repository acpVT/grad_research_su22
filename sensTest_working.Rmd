...
sens_un <- function(data, init, v){
  ...
  for(i in 1:3){
    sd <- sd(data[,i])
    for(n in 1:length(data[,1])){
      copy[n,i]<-abs(((data[n,i]-data[6,i])/data[6,i])*(((data[n,4]-init)/init)^-1)) # Approximation of partial derivative
      copy[n,i]<-copy[n,i] * 2*sd
    } # Loops through columns then rows
  }
  var <- v
  a <- mean(copy[,1]) # Take the mean SOM_1
...
```{r}
SD_df <- data.frame(matrix(nrow = 11, ncol = 4))
