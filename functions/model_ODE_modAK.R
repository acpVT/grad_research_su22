...
model_ODE_modAK <- function(data, times){
  ...
  siteSpecificParameters = function() {
    ...
    k        <<- LTERdata$k[s]    #2.0			#REDUCED FROM 3 TO 1, REDUCES TEXTURE EFFECTS ON SOMa decay
    a        <<- LTERdata$a[s]    #2.2			#increased from 4.0 to 4.5
